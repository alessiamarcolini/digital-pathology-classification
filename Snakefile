...
CORRECT_TILES_CSV_FILENAME = DATA_DIR / 'correct_tiles.csv'
...
rule recompact_correct_tiles:
    input:
        ...
        correct_tiles_summaries = expand('{tiles_per_svs_dir}/{svs_filename_no_ext}/correct_tiles_per_svs_filenames.csv', tiles_per_svs_dir=TILES_PER_SVS_DIR,  svs_filename_no_ext=SVS_filenames_no_ext)
    output:
        directory(TILES_DIR),
        CORRECT_TILES_CSV_FILENAME
    shell:
        'python recompact_correct_tiles.py --tiles_dirs {input.tiles_dirs} --correct_tiles_summaries_path {input.correct_tiles_summaries} --output_tiles_folder {output[0]} --correct_tiles_csv_path {output[1]}'
